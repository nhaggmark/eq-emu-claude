...
| B | C++ refactor: mirror-pipeline approach across mob.h, mob.cpp/attack.cpp, companion.h, companion.cpp, exp.cpp:1196-1218, lua_companion.cpp; ruletypes.h `Companions:XPSharePct` default 50→100; AA-seam comment in `Companion::CalculateExp`. Build clean. | c-expert | Complete (2026-04-27) | Commit a0114be44 on feature/xp-retune. 244/244 clean, zero warnings. `Companion::CalculateExp(uint32, uint8)` symbol verified in zone binary. |
| C | Restart server stack: rebuild eqemu, then loginserver → world → 8 dynamic zone processes (`dynamic_01` through `dynamic_08`) per `MEMORY.md` startup order. Verify all 8 zones connect cleanly. | infra-expert | Complete (2026-04-27) | Binary verified (Companion::CalculateExp present, timestamp 18:03). make restart clean. shared_memory complete. loginserver + world + 8 dynamic zones all running. Zone count verified: 8. world.log clean, no XP-path exceptions. |
| D | Apply companion XPSharePct UPDATE: pre-check SELECT (expect `'50'`) → UPDATE to `'100'` → post-check SELECT → `#reloadrulesworld` in-game. Verify post-rebuild stack health before applying. | config-expert | Complete (2026-04-27) | Pre-check: `'50'`; UPDATE confirmed 1 row; Post-check: `'100'`. Gate: infra-expert confirmed new binary with Companion::CalculateExp symbol present. Reload needed in-game. Migration artifact at `config-expert/context/task-d-migration.sql`. |
