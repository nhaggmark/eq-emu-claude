...
3. **A_Spiritual_Arcanist 154153 (L68, 150k HP, raid_target=1, acrylia, script-spawned) audit-missed — Decision #59 refined per lore-master 2026-04-25.** She is the **Phase 2 "wrong choice penalty" combat target** in the Khati Sha event chain (NOT one of the three outer AC raids — Ring of Fire / Vah Shir Captive / Burrower remain separate, IDs unconfirmed). Per `#Raidman.lua`: spawns alongside Arcanist V1 (154151, 75k, raid_target=0) + V2 (154152, 75k, raid_target=0); player must `/tar a_spiritual_arcanist00` to pick correct rescue target. Wrong choice → 4 deathguards spawn + fight 154153. Architect Phase 5a target HP refined: **150k → 40k** (matches lore-master suggestion "150k → ~40k reasonable for wrong-choice penalty"). 154151/154152 NOT in Phase 5a scope (raid_target=0 quest NPCs). Decision #59 OPEN pending user confirmation of refined HP target.
