...
| Architecture | architect + protocol-agent + config-expert + lore-master | Phase 2-4b: all Complete 2026-04-23. **Phase 5a Luclin non-VT: Complete 2026-04-23** — protocol-agent + config-expert cleared 2026-04-22; lore-master sign-off pending (luclin-chains.md serves as primary lore reference). 37 npc_types UPDATEs + 1 npc_spells_entries DELETE (Touch of Vinitras DT). 3 user-decision items surfaced (#50/#51/#52). Ready for implementation dispatch. | 2026-04-22 | 2026-04-23 |
...
| Phase 5a — Luclin non-VT | Ssraeshza Temple (13+2 flagged serpents), Akheva Ruins (8 incl. 3 Vyzh\`dra variants + Shei Vinitras dual-form), Sanctus Seru/Katta (7), Grieg's End (3), Acrylia (2 incl. Khati Sha), The Deep (1), Umbral (3 incl. Doomshade) = 37 NPCs. Touch of Vinitras DT removal (1 spell). | **Architecture Complete 2026-04-23** — awaiting user decisions #50/#51/#52 + lore-master sign-off |
...
|---|------|---------------|--------|-------|
| L1 | Build 3 backup tables (`npc_types_backup_raid_scaling_luclin_a` 37 rows, `spawn2_backup_raid_scaling_luclin_a` ~16-18 rows, `npc_spells_entries_backup_raid_scaling_luclin_a` 1 row); verify counts | data-expert | **Not Started** | Mirrors Phase 4b backup pattern with `_luclin_a` suffix |
...
| 50 | Phase 5a: rune/glyph-covered serpents (162253 221k HP / 162261 300k HP, ssratemple, raid_target=1, audit-missed, part of `#cursed_controller.pl` chain) — Option A (INCLUDE per architect default, scale to 60k/70k HP) or Option B (exclude per Decision #2 elite-trash interpretation)? | architect | user | **Open** | Architect recommends Option A — protocol-agent Flag C identified as Vyzh\`dra-chain stepping stones. Lore-master sign-off pending. |
...
| 50 | **Phase 5a: rune/glyph serpents architect-recommends INCLUDE.** Per protocol-agent Flag C (`#cursed_controller.pl` chain orchestration), 162253 (rune-covered serpent, 221k HP) and 162261 (glyph-covered serpent, 300k HP) appear to be Vyzh\`dra-chain stepping stones. Architect default INCLUDE: scale to 60k/70k HP. Lore-master sign-off pending. | architect | 2026-04-23 | **USER APPROVAL REQUIRED.** |
