...
| Architecture | architect + protocol-agent + config-expert + lore-master | Phase 2-4b: all Complete 2026-04-23. **Phase 5a Luclin non-VT: Complete 2026-04-25** — all advisors cleared (protocol-agent + config-expert 2026-04-22; **lore-master 2026-04-25 — APPROVED with Q1-Q17 sign-off**). 37 npc_types UPDATEs + 1 npc_spells_entries DELETE (Touch of Vinitras DT in list 196 only — Shei list 179 PRESERVED per lore-master Q15). 3 prior user-decision items + 1 new (Decision #50 RESOLVED Option A joint recommendation; Decision #59 surfaced — A_Spiritual_Arcanist 154153 audit-missed). Ready for implementation dispatch. | 2026-04-22 | 2026-04-25 |
...
| Phase 5a — Luclin non-VT | Ssraeshza Temple (13+2 cycle serpents per Decision #50 RESOLVED), Akheva Ruins (8 incl. 3 Vyzh\`dra variants + Shei Vinitras dual-form), Sanctus Seru/Katta (7), Grieg's End (3), Acrylia (2 incl. Khati Sha + 1 newly-discovered Spiritual Arcanist Decision #59), The Deep (1), Umbral (3 incl. Doomshade) = 38 NPCs default. Touch of Vinitras DT removal scoped to list 196 only (Shei list 179 preserved per Decision #60). | **Architecture Complete 2026-04-25** — lore-master APPROVED; awaiting user decisions #51/#52/#59 |
...
|---|------|---------------|--------|-------|
| L1 | Build 3 backup tables (`npc_types_backup_raid_scaling_luclin_a` 38 rows incl. Decision #50 resolved + Decision #59 default include, `spawn2_backup_raid_scaling_luclin_a` ~16-18 rows, `npc_spells_entries_backup_raid_scaling_luclin_a` 1 row); verify counts | data-expert | **Not Started** | Mirrors Phase 4b backup pattern with `_luclin_a` suffix. Decision #50 resolved Option A (rune/glyph serpents IN); Decision #59 adds A_Spiritual_Arcanist 154153 pending user approval. |
...
| L3 | Emit per-boss HP/damage UPDATE SQL for akheva cluster (3 Vyzh\`dra variants + Itraer Vius + Shei Vinitras REAL 179032 + MERCHANT 179157 + Insanity Crawler + Va\`Dyn + Shar Vinitras = 8 NPCs) | data-expert | **Not Started** | Sheleric Vis + Xaui Tatrua excluded per Decision #51 default exclude |
...
| L6 | Emit per-boss HP/damage UPDATE SQL for griegsend (3) + acrylia (2 incl. Khati Sha 154145 acrylia-confirmed) + thedeep (1 Thought Horror Overfiend) + umbral (3 incl. Doomshade 176088 audit-missed) = 9 NPCs | data-expert | **Not Started** | Excludes 163051/52 (LoN OOE), 154161 (Fabled), 176111 (Netherbian L73 OOE), 176110 Keymaster, 153095 Blaystich (elite-named tier per Decision #2) |
...
| L11 | Smoke verify: HP targets for Emperor (120k) / Lord Seru (120k MR=800) / Vyzh\`dra Cursed (90k) / Khati Sha (90k) / Doomshade (70k) / Thought Horror Overfiend (90k); respawn 24h on ~17-18 rows; **Touch of Vinitras DELETE confirmed** (`SELECT COUNT(*) FROM npc_spells_entries WHERE npc_spells_id=196 AND spellid=2859` returns 0); Spirit of Akelha\`Ra (179144) UNTOUCHED at 1M HP; vexthal NPCs (158081 + 158087-94) UNTOUCHED; OOE NPCs (163051/52, 154161, 176111) UNTOUCHED; event-control NPCs (162269, 176110, 160177/178, 162065, 162260) UNTOUCHED | config-expert | **Not Started** | — |
...
| L13 (CONDITIONAL) | **ONLY IF user picks Decision #52 = Option B:** perl-expert edits `akk-stack/server/quests/ssratemple/#EmpCycle.pl:3` to change `$EmpRepopTime = int(rand(2880)) + 4320;` (3-5d) to `$EmpRepopTime = int(rand(7200)) + 79200;` (22-24h). Commit on feature/raid-scaling. Architect strongly recommends Option A (skip). | perl-expert | **Conditional — Not Started by default** | Per Decision #11 + #45 Thylex precedent |

**Required implementation agents (default path):** data-expert + config-expert. (perl-expert only if Decision #52 = Option B.)
...
| 51 | Phase 5a: Akheva elite-named (Sheleric Vis 179133/179046, Xaui Tatrua 179044, all 70-116k HP / 5400s respawn) — Option A (EXCLUDE per Decision #2 elite-trash, architect-recommended; same posture as Phase 4b Defenders pre-override) or Option B (include for scope consistency, akin to Q37 user override)? | architect | user | **Open** | Architect recommends Option A. |
| 52 | Phase 5a: Emperor Ssraeshza cycle respawn (`#EmpCycle.pl:3` `$EmpRepopTime` 3-5 day) — Option A (KEEP NATIVE per Decision #11 + #45 Thylex precedent, architect-strongly-recommended) or Option B (invoke perl-expert task L13 to soften to 22-24h endgame tier)? | architect | user | **Open** | Architect strongly recommends Option A — 3-5 day cadence is signature feel. perl-expert task L13 conditional only on Option B. |
| 59 | Phase 5a: A_Spiritual_Arcanist 154153 (L68, 150k HP, raid_target=1, acrylia, script-spawned, no spawn2) — audit-missed surfaced during lore-master review. Option A (INCLUDE in Phase 5a, scale to 45k HP, architect default) or Option B (exclude pending lore-master deeper investigation)? | architect | user | **Open** | Architect default Option A. Likely an Acrylia inner-zone scripted raid; further lore investigation could clarify role. |
...
| 59 | **A_Spiritual_Arcanist 154153 (acrylia, L68, 150k HP, raid_target=1, script-spawned, no spawn2) audit-missed — surfaced during lore-master review 2026-04-25.** Architect adds Phase 5a default INCLUDE with HP target 45k (between Khati Sha 90k and Defiled Minion 5k). Likely Acrylia inner-zone scripted raid distinct from Khati Sha event (different ID block from 154054 Defiled Minion / 154129 diseased grimling adds). | architect | 2026-04-25 | **USER APPROVAL REQUIRED.** Lore-master confirmed acrylia inner raids exist (Ring of Fire / Vah Shir Captive / Burrower) but had no PEQ NPC IDs; this may be one. |
