...
| V2-2 | Apply Lua fix in `companion.lua` — (a) rename `check_existing_companion_record` param to `clean_name`, swap SQL predicate to `name = ?`, change caller at line 463 to pass `npc:GetCleanName()`, update doc comment; (b) add Q7 mitigation: new exclusion check in `is_re_recruitment_eligible()` calling shared `_lookup_exclusion(npc:GetNPCTypeID())` helper (refactor from `is_eligible_npc()` if not shared); run `make test-companion`; verify 3 new tests pass + 58 v1 tests pass | lua-expert | Not Started | Depends on V2-1 |
| V2-3 | Add Suite 35 (`TestCompanionReRecruitmentVariantNameMatch`) in `cli_companion_tests.cpp` — three test cases: (a) name-match finds row when ID differs, (b) name-mismatch returns empty, (c) Q7 mitigation — exclusion check on target NPC blocks Track 1 even when name matches | c-expert | Not Started | Tests must fail BEFORE V2-4 per AC-9 |
| V2-4 | Apply C++ fix at `companion.cpp:218-220` (replace `npc_type_id={}` with escaped `name='{}'`, add ORDER BY tie-breaker, update comment block) AND add Q7 mitigation: exclusion check on `source_npc->GetNPCTypeID()` BEFORE `Load()` in re-recruit hit path; rebuild zone via ninja | c-expert | Not Started | Depends on V2-3 |
