...
| 2 | Identify existing Necromancer summon-corpse row in `spells_new` for icon/animation/descnum clone-mutate values; assign new unique `spell_category` value for the 12 new spells; share constant with c-expert | data-expert | Complete | 2026-05-03. spell_category=221. Clone from spell 2213. c-expert notified. |
| 3 | Author 12 INSERT statements for new spells in `spells_new` (using assigned IDs `1348, 5093, 9412–9421` per architecture §Data Model, cloned cosmetic fields from task 2, and the new spell_category) | data-expert | Complete | 2026-05-03. In feature_summon_corpse_spell.sql. Verified in DB. |
| 4 | Add `m_summon_corpse_was_noop` flag (`mob.h`); set in SummonCorpse no-corpse branch (`spell_effects.cpp:1851`); read in recast block (`spells.cpp:2817-2841`) gated on `spell_category == kUniversalSummonCorpseCategory`; add `RuleI(Spells, UniversalSummonCorpseCooldown)` override in same block | c-expert | Complete | 2026-05-03. Commits 5f5e69570 (Phase A) + 8a5f4565e (Phase B). Build verified (ninja 2 steps, clean link). |
| 5 | Author 12 INSERT statements for scroll items in `items` | data-expert | Complete | 2026-05-03. Item IDs 1000001-1000012. itemtype=20 (live DB correction). |
| 6 | Enumerate starting-city class spell vendor merchant_ids; author merchantlist INSERTs | data-expert | Complete | 2026-05-03. 57 vendor entries across all starting cities. |
